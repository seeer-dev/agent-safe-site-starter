...
observed bucket is `agent-safe-site-starter-assets`; replace only the bucket
...
| `R2_BUCKET` | **R2 物件儲存（R2 Object Storage）** → **概觀（Overview）** → 表格欄位 **貯體（Buckets）** | `agent-safe-site-starter-assets` | **Text**。 |
| `R2_PUBLIC_BASE_URL` | `agent-safe-site-starter-assets` → **設定（Settings）** → **公用開發 URL（Public Development URL）** → **啟用（Enable）**（測試）；正式環境改用 **自訂網域（Custom Domains）** → **新增（Add）** | Cloudflare 顯示的 **Public Bucket URL** 或已連線的自訂圖片網域；移除結尾 `/` | **Text**：Railway 與 storefront Pages 的 **Production build** 各一份。不要設在 admin 或任何 Pages Function。 |

建立 Account API 權杖時，選 **Object Read & Write**，並限制為
**只套用指定 bucket** 的 `agent-safe-site-starter-assets`。兩個 key 是
