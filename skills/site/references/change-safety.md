...
8. When a broad change suggests a new module/provider/framework, first prove the current local seam cannot safely solve the real requirement. Prefer consumer-owned typed ports, cohesive internal splits, and the existing provider path over speculative platform abstractions.
...
CodeGraph is context discovery, not write authorization. `architecture.yaml` supplies the import policy consumed by `archcheck`; `speccheck`, the scope gate, architecture gate, and CI provide enforcement.
