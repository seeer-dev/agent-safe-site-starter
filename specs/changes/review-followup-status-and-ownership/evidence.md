...
| ID | Status | Proof |
| --- | --- | --- |
| REQ-001 | passed | docs/project-status.md now distinguishes implemented commerce/payment behavior from HTTP contract truth, makes contract restoration a v1 pre-deploy blocker, and keeps external stage/deploy acceptance separate. |
| AC-001 | passed | The canonical status explicitly says source-level commerce can be implemented while runtime/OpenAPI contract truth still needs restoration before v1 deploy-ready acceptance. |
...
| REQ-003 | passed | docs/review-status.md records the current interpretation of historical reviews, evidence-complete lifecycle closures versus still-pending changes, and the revised priority order; docs/README.md routes readers to it. |
