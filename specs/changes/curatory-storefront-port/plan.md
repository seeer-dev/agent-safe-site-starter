...
Pre-existing dirty paths: none (fresh baseline commit).
...
- Paired migrations (sqlite + postgres): `categories`, `product_variants`, `product_comments`, `promos` extension (`freeshipping` type, `min_subtotal`, `usage_limit`, `used_count`), `orders` extension (`order_number` if distinct, `recipient_name`, `recipient_phone`, `city`, `district`, `cvs_store_name`, `cvs_store_address`, `invoice_type`, `invoice_tax_id`, `payment_fee`, `coupon_code`, `buyer_note`, `completed` status), `store_settings` single-row record in `sitecontent`.
- Commerce model/service/store updates: variants, categories, featured flag, sold count, comment store surface, coupon validation + order application, order field persistence.
...
### S02 — Public API surface

Covers: REQ-004, REQ-005, REQ-006, REQ-007, REQ-008, REQ-009, REQ-011

- `GET /api/storefront/bootstrap` — settings + categories + shipping + payment + announcements summary.
- Product list filters (`featured`, `sort`, `category`, `q`) and product detail including variants/images/comments-count.
- `POST /api/coupons/validate`.
- `GET/POST /api/products/{slug}/comments` (public list approved; submit → pending).
- `GET /api/news`, `GET /api/news/{slug-or-id}` (or reuse article endpoints) honoring pinned/publish-at.
- Order create extension + guest access-token fetch (existing), order-confirmation mail via mailer seam.
- `contracts/openapi.yaml` updated for every endpoint.

Expected evidence: API tests for validation, masking, coupon edge cases; contract check.

### S03 — Theme scaffold `site/themes/curatory/`

Covers: REQ-001, REQ-003 (partial)

- Vite/Tailwind config, design tokens ported from reference globals, `bootstrap.ts` island mounter (same pattern as minimal-cart), shared `api` client, `cart` store (variant-aware, localStorage `curatory-cart`), `format` helpers.
...
- Theme checks scripts patterned on minimal-cart (`check:browser-authority` etc. as applicable).
...
- `home.html` + islands: Header, PromoBanner, Hero, FeaturedProducts, ProductGrid (filter/search/sort), Footer.
- `shop.html` with cat/q/sort params.
- `product.html` + islands: gallery, variant selector, add-to-cart, comments list + submit form.
- Server-rendered baselines for SEO/no-JS; islands enhance only after authoritative load.

Expected evidence: rendered HTML inspection + walkthrough receipt.
...
- `checkout.html` + checkout island: Taiwan city/district cascade, CVS store picker (static ported store data), invoice type/tax id, recipient fields, coupon, note; server quote display.
- `order.html` confirmation (consumes access token from session storage), `track.html` (token-based retrieval).
...
### S07 — Admin SPA resources

Covers: REQ-010

- `admin/src/config/resources/`: categories, announcements/news, coupons, comments (moderation actions), settings; product form variant editor.
- Route/nav registration; capability gating wired to `/api/admin/me`.

Expected evidence: admin walkthrough receipt per resource (permitted + denied).

### S08 — Content seed + end-to-end verification

Covers: REQ-001–REQ-012 closure

- Dev seed extended: categories, variants, featured, announcements, coupons, settings matching reference demo data.
...
| REQ-003 / AC-004 | S03–S05 | shared components | walkthrough receipt |
| REQ-004 / AC-005, AC-006 | S01, S02, S04 | variants, categories, filters | API tests + UI walkthrough |
| REQ-005 / AC-007, AC-008 | S01, S02, S05 | quote/order/checkout | API tests + walkthrough |
| REQ-006 / AC-009, AC-010 | S01, S02, S05 | orders | security-review + walkthrough |
| REQ-007 / AC-011 | S01, S02, S06 | news pages | render inspection |
| REQ-008 / AC-012 | S01, S02, S04, S07 | comments | security-review + moderation walkthrough |
| REQ-009 / AC-013 | S01, S02, S05, S07 | coupons | API edge-case tests |
| REQ-010 / AC-014 | S07 | admin SPA | admin walkthrough |
| REQ-011 / AC-015 | S01, S02, S03 | bootstrap endpoint | consumer-reachability receipt |
| REQ-012 / AC-016, AC-017 | S01, S08 | migrations, gates | migration-parity, speccheck, verify |
